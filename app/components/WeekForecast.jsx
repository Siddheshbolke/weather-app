...
const WeekForecast = ({data}) => {
  return (
    ...
    {data.forecast.forecastday.map((day,index)=>(
      <div key={index} className="bg-white/40 p-2 text-center rounded-lg flex flex-col items-center font-semibold gap-4" role='group' aria-label={`Forecast for ${new Date(day.date).toLocaleDateString("en-US",{weekday:"short"})}`} >
